...
class SlotAvailabilityService
{
    ...
    public function isVacation($id_store, $id_shop, $dateString)
    {
        $cacheKey = $id_store . '_' . $id_shop;
        if (!isset($this->vacationsCache[$cacheKey])) {
            $this->vacationsCache[$cacheKey] = $this->getVacations($id_store, $id_shop);
        }
        
        $vacations = $this->vacationsCache[$cacheKey];
        if (empty($vacations)) {
            return false;
        }
        
        $checkTime = strtotime($dateString);
        foreach ($vacations as $v) {
            $start = strtotime($v['vacation_start'] . ' 00:00:00');
            $end = strtotime($v['vacation_end'] . ' 23:59:59');
    ...
    public function getAvailableSlotsForDate($id_store, $dateString, $id_day, $id_shop)
    {
        ...
        if ($this->isVacation($id_store, $id_shop, $dateString)) {
            return [];
        }
        ...
        // Uniquement appliqué sur la date demandée s'il s'agit d'aujourd'hui
        if ($dateString === $now->format('Y-m-d') && $delay_minutes > 0) {
