...
class Dfs_clickcollectAdminajaxModuleFrontController extends ModuleFrontController
{
    public $ajax = true;
    ...
    public function initContent()
    {
        ...
        if ($token !== Tools::getAdminTokenLite('AdminDfsClickCollect')) {
            die(json_encode(['success' => false, 'message' => 'Token invalide. Accès refusé.']));
        }
        ...
        if ($id_creneau) {
            ...
            // Determine context shop
            $id_shop = (new Shop(1))->id ? 1 : 1;
            ...
            if (Db::getInstance()->execute($insertSql)) {
                die(json_encode(['success' => true, 'message' => 'Créneau associé avec succès.']));
            }
        }

        die(json_encode(['success' => false, 'message' => 'Erreur inconnue lors de l\'enregistrement.']));
    }
}
